...
async function setup() {
  ...
  try {
    ...
    const defaultWelcome = JSON.stringify({
      enabled: true,
      message: "Welcome to the server, {user}! Enjoy your stay.",
      channelId: "",
    });
    const defaultGoodbye = JSON.stringify({
      enabled: false,
    ...
    const settingsAttributes = [
      ...
      },
      {
        ...
        size: 2000,
        required: false,
        default: defaultGoodbye,
      },
      {
        key: "autoRoleSettings",
        type: "string",
        size: 500,
        required: false,
        default: defaultAutoRole,
      },
      {
        key: "levelingSettings",
        type: "string",
        size: 10000,
        required: false,
        default: defaultLeveling,
      },
      {
        key: "autoModSettings",
        type: "string",
    ...
    const youtubeAttributes = [
      ...
      {
        key: "lastAnnouncedVideoId",
        type: "string",
        size: 64,
        required: false,
      },
      {
        key: "lastAnnouncedVideoTitle",
        type: "string",
        size: 256,
        required: false,
      },
    ...
    await createIndex(APPWRITE_DATABASE_ID, REACTION_ROLES_COLLECTION_ID, {
      key: "guildId_messageId_key",
      type: IndexType.Key,
      attributes: ["guildId", "messageId"],
      orders: ["ASC", "ASC"],
    });
    ...
    await createIndex(APPWRITE_DATABASE_ID, SCHEDULED_MESSAGES_COLLECTION_ID, {
      key: "status_nextRun_key",
      type: IndexType.Key,
      attributes: ["status", "nextRun"],
      orders: ["ASC", "ASC"],
    });
    ...
    await createIndex(APPWRITE_DATABASE_ID, GIVEAWAYS_COLLECTION_ID, {
      key: "status_endsAt_key",
      type: IndexType.Key,
      attributes: ["status", "endsAt"],
      orders: ["ASC", "ASC"],
    });
    ...
    for (const attr of [
      { key: "guildId", type: "string", size: 32, required: true },
      { key: "reactionRoleId", type: "string", size: 32, required: true },
    ])
      await createAttribute(
        APPWRITE_DATABASE_ID,
        REACTION_ROLE_QUEUE_COLLECTION_ID,
    ...
    for (const attr of [
      { key: "guildId", type: "string", size: 32, required: true },
      { key: "giveawayId", type: "string", size: 32, required: true },
    ])
      await createAttribute(
        APPWRITE_DATABASE_ID,
        GIVEAWAY_QUEUE_COLLECTION_ID,
    ...
    await createCollection(
      APPWRITE_DATABASE_ID,
      MUSIC_QUEUE_COLLECTION_ID,
      "Music Queue",
      [Permission.create(Role.users())]
    );
    for (const attr of [
      { key: "guildId", type: "string", size: 32, required: true },
      { key: "songUrl", type: "string", size: 512, required: true },
      { key: "requesterId", type: "string", size: 32, required: true },
    ])
      await createAttribute(
        APPWRITE_DATABASE_ID,
        MUSIC_QUEUE_COLLECTION_ID,
