  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const weeklyChartData = processChartData(
          Array.isArray(statsData.messagesWeekly)
            ? statsData.messagesWeekly
            : []
        );
          ...
          .map((log: LogEntry) => {
            ...
            switch (log.type) {
              case LogType.AI_MODERATION:
                logText = (
                  <>
                    <span className="font-bold">{log.user}</span> had a message
                    flagged by AI.
                  </>
                );
                icon = ICONS.shield;
              ...
              case LogType.AUTO_MOD_ACTION:
                logText = (
                  <>
                    <span className="font-bold">AutoMod</span> took action on{" "}
              ...
              case LogType.MessageDeleted:
                logText = (
                  <>
                    <span className="font-bold">{log.user}</span> triggered a
              ...
              default:
                logText = (
                  <>
                    <span className="font-bold">{log.user}</span> triggered a{" "}
                    {log.type.toLowerCase().replace(/_/g, " ")} event.
                  </>
                );
            }
        ...
        const mappedCommandLogs: ActivityItem[] = commandLogs
          ...
          .map((log: CommandLogEntry) => ({
            ...
            text: (
              <>
                <span className="font-bold">{log.user}</span> used command{" "}
  ...
  if (!stats) {
    return (
      <div className="text-center text-text-secondary">
        Could not load server statistics for {selectedServer.name}.
      </div>
    );
  }
