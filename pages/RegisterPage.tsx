...
const FeatureHighlight: React.FC<{icon: React.ReactNode, title: string, description: string}> = ({ icon, title, description }) => (
  <div className="flex items-start space-x-4">
    <div className="flex-shrink-0 bg-white/10 p-3 rounded-lg text-accent">{icon}</div>
    <div>
...
const EyeSlashIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3.707 2.293a1 1 0 00-1.414 1.414l14 14a1 1 0 001.414-1.414l-1.473-1.473A10.014 10.014 0 0019.542 10C18.268 5.943 14.478 3 10 3a9.958 9.958 0 00-4.512 1.074l-1.78-1.781z" clipRule="evenodd" /><path d="M12.454 16.697l-1.414-1.414a3 3 0 01-4.242-4.242L6.758 10.982a4.978 4.978 0 00-1.32 1.018A10.005 10.005 0 00.458 10c1.274 4.057 5.022 7 9.542 7 .848 0 1.67-.11 2.46-.317z" /></svg>;
const DiscordIcon = () => (
...
const RegisterPage: React.FC = () => {
    ...
    const [loadingProvider, setLoadingProvider] = useState<'email' | 'discord' | null>(null);
    const { register, user, authError, setAuthError, loginWithDiscord } = useAuth();
            ...
            <div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                <div className="w-full max-w-md space-y-8">
                    <div className="text-center">
                        <img src="/img/discord-logo.png" alt="Discora Logo" className="mx-auto h-16 w-16"/>
                        <h2 className="mt-6 text-3xl font-bold tracking-tight text-text-primary">Create an Account</h2>
                        <p className="mt-2 text-text-secondary">Get started with your new mission control.</p>
                    </div>
                    
                    {authError && (
                        <div className="bg-red-500/10 border border-red-500/30 text-red-300 px-4 py-3 rounded-md text-center text-sm" role="alert">
                           {authError}
                        </div>
                    )}
                    ...
                    <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        disabled={loadingProvider !== null}
                                    />
                                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-text-secondary hover:text-text-primary">
                        ...
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center text-sm"><span className="px-2 bg-background text-text-secondary">Or sign up with</span></div>
                        </div>
            ...
            <div className="hidden lg:flex flex-col items-center justify-center p-12 bg-gradient-to-br from-secondary to-background border-l border-white/10">
                 <div className="w-full max-w-md">
                     <div className="space-y-8">
                        <h2 className="text-3xl font-bold text-white">Your All-in-One Mission Control</h2>
                        <p className="text-text-secondary">Transform server management from a series of text commands into an intuitive and powerful visual experience.</p>
                        <div className="space-y-6">
                            <FeatureHighlight icon={ICONS.dashboard} title="Real-time Dashboard" description="Get a bird's-eye view of your server with stats on members, messages, and more." />
                            <FeatureHighlight icon={ICONS.autoMod} title="Advanced Auto-Mod" description="Configure filters and leverage AI to keep your community safe and clean." />
                            <FeatureHighlight icon={ICONS.commands} title="Custom Commands" description="Easily create, edit, and delete custom text or embed commands for your server." />
                        </div>
                    </div>
                 </div>
            </div>
