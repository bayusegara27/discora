...
const YoutubePage: React.FC = () => {
    ...
    const [editingSub, setEditingSub] = useState<YoutubeSubscription | null>(null);
    ...
    const defaultCustomMessage = "📢 Hey @everyone! {channelName} just uploaded a new video!\n\n**{videoTitle}**\n{videoUrl}";
    const defaultLiveMessage = "🔴 Hey @everyone! {channelName} is now LIVE!\n\n**{videoTitle}**\n{videoUrl}";
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!currentSub.youtubeChannelId || !currentSub.discordChannelId) {
            addToast("YouTube Channel ID and an Announcement Channel are required.", "error");
            return;
        }
        setSaving(true);
        try {
            if (editingSub) {
                await appwriteService.updateYoutubeSubscription({ ...editingSub, ...currentSub } as YoutubeSubscription);
                ...
                addToast('Subscription created! The bot will start monitoring.', 'success');
            }
            fetchData(selectedServer.guildId);
            closeModal();
        } catch (error) {
    ...
    if (!selectedServer) {
        return <div className="text-center text-text-secondary">Please select a server to manage YouTube notifications.</div>;
    }
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-text-secondary">Manage video upload announcements for {selectedServer.name}.</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-left">
                        <thead className="border-b border-gray-700">
                            <tr>
                                <th className="p-4 text-sm font-semibold text-text-secondary">YouTube Channel</th>
                                <th className="p-4 text-sm font-semibold text-text-secondary">Post to Discord Channel</th>
                                <th className="p-4 text-sm font-semibold text-text-secondary">Last Video Found</th>
                                <th className="p-4 text-sm font-semibold text-text-secondary text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {subscriptions.length > 0 ? subscriptions.map(sub => (
                                <tr key={sub.id} className="border-b border-gray-800 hover:bg-background">
                                    <td className="p-4 align-top">
                                        <div className="font-medium text-text-primary">{sub.youtubeChannelName || '(Name pending...)'}</div>
                                        <div className="font-mono text-xs text-text-secondary">{sub.youtubeChannelId}</div>
                                    </td>
                                    <td className="p-4 align-top">
                                        <div className="font-medium text-text-primary">{sub.discordChannelName ? `#${sub.discordChannelName}` : '(Name pending...)'}</div>
                                    ...
                                    <td className="p-4 align-top text-right space-x-2 whitespace-nowrap">
                                        <button onClick={() => openModalForEdit(sub)} className="text-blue-400 hover:text-blue-300">Edit</button>
                                        <button onClick={() => handleDelete(sub.id!)} className="text-red-400 hover:text-red-300">Delete</button>
                                    </td>
                                </tr>
                            )) : (
                                <tr>
                                   <td colSpan={4} className="text-center p-8 text-text-secondary">No YouTube subscriptions created for {selectedServer.name}.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
