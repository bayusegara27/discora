...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    const roleColor =
      data.payload.color !== "#000000" ? data.payload.color : "#99aab5";
    return (
      <div className="bg-surface/80 backdrop-blur-sm border border-primary p-3 rounded-lg shadow-lg text-sm">
...
const RoleDistributionChart: React.FC<RoleDistributionChartProps> = ({
  ...
  const totalMembersWithRoles = chartData.reduce(
        ...
        <PieChart>
          ...
          >
            {chartData.map((entry, index) => (
              <Cell
                key={`cell-${index}`}
                fill={entry.color !== "#000000" ? entry.color : "#99aab5"}
                stroke={entry.color}
              />
            ))}
          </Pie>
